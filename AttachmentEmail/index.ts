...
export class AttachmentEmail implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	...
	public async init(context: ComponentFramework.Context<IInputs>, notifyOutputChanged: () => void, state: ComponentFramework.Dictionary, container: HTMLDivElement) {
		...
		// Si on est sur une création d'email
		// var i = 0;
		// while(!(<any>context).page.entityId && i < 20){
	...
	private onClickConfirmDelete(attachment: AttachmentRef){
		...
		this._context.webAPI.deleteRecord("activitymimeattachment", id).then(
			function success(result) {
			console.log("Successfully deleted the record");
			return result;
			}
		).then(result => {
				this.removeBSCard(result.id.toString());
		})
	}
	...
	private async getAttachments(reference: EntityReference): Promise<Attachment[]> {
		...
		let query = `?$select=activitymimeattachmentid,mimetype,filename,filesize&$filter=filename ne null and _objectid_value eq ${reference.id} and objecttypecode eq '${reference.typeName}' &$orderby=filename desc`;
		try {
			...
			for (let i = 0; i < result.entities.length; i++) {
				...
				let item = new Attachment(new EntityReference("activitymimeattachment", ent["activitymimeattachmentid"].toString()), ent["filename"].split('.')[0], ent["filename"].split('.')[1].toLowerCase(), false);
	...
	private getAttachment(id: string, type: string): Promise<FileToDownload> {
		...
		return this._context.webAPI.retrieveRecord("activitymimeattachment", id).then(
			function success(result) {
				let file: FileToDownload = new FileToDownload();
				file.fileContent = result["body"];
